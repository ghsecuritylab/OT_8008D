					    ...
					    struct wpa_ssid *group)
{
	...
	for (ssid = group; ssid; ssid = ssid->pnext) {
        ...
        /* FR391432 BEGIN---Change WIFI 3G fall back threshold---QIHONG.XU@TCL.COM---Jan 23,2013 */
        //update this threshold for PR399470 by songfang
        char wifi_connect_threshold[10];
        property_get("ro.wifi.connect.threshold", wifi_connect_threshold, "-85");
        if(bss->level > 0 && bss->level - 256 < atoi(wifi_connect_threshold)) {
            ...
            wpa_printf(MSG_DEBUG, "connect_threshold %d",atoi(wifi_connect_threshold));
            wpa_printf(MSG_DEBUG, " skip - rssi weak %d",bss->level);
            continue;
        }
        /* FR391432 END---Change WIFI 3G fall back threshold---QIHONG.XU@TCL.COM---Jan 23,2013 */
#ifdef CONFIG_WPS
...
//PR399469 fang.song-001 delete one line
static struct wpa_bss *
			    ...
			    struct wpa_ssid **selected_ssid)
{
	...
	while (selected == NULL) {
            //PR399469 fang.song-001 modified begin
            char ssid_priority_support[1];
            property_get("ro.wifi.ssid.priority.support", ssid_priority_support, "0");
            if(strcmp(ssid_priority_support, "1") == 0){
            //PR399469 fang.song-001 modified end
