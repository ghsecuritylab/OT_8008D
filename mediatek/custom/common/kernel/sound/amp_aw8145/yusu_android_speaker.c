...
static bool hac_on=false; //ting.kang add for HAC function 
...
void Sound_HAC_Turnon(void)
{
	...
	if(hac_on)
		return;
    mt_set_gpio_mode(GPIO_HAC_PA_EN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_HAC_PA_EN,GPIO_DIR_OUT); 
    mt_set_gpio_out(GPIO_HAC_PA_EN,GPIO_OUT_ONE); 
    msleep(SPK_WARM_UP_TIME);
    hac_on = true;
}
...
void Sound_HAC_Turnoff(void)
{
    ...
    mt_set_gpio_mode(GPIO_HAC_PA_EN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_HAC_PA_EN,GPIO_DIR_OUT); 
    mt_set_gpio_out(GPIO_HAC_PA_EN,GPIO_OUT_ZERO); 
	hac_on = false;
}
