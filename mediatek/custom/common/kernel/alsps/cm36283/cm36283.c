...
long cm36283_read_als(struct i2c_client *client, u16 *data)
{
	long res;
	u8 databuf[2];
	u16 databuf1[2];  //added by zhengdaoli for patching mtk bug
	...
	APS_LOG("CM36283_REG_ALS_DATA value value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
	    databuf1[0] = databuf[0]; //added by zhengdaoli for patching mtk bug
		databuf1[1] = databuf[1]; //added by zhengdaoli for patching mtk bug

	*data = ((databuf1[1]<<8)|databuf1[0]); //changed by zhengdaoli for patching mtk bug
	printk(" rawdata_als = %d\n", *data);     //added by zhengdao for debugging
...
static unsigned long long eint_time = 0;
/*----------------------------------------------------------------------------*/
static int cm36283_check_intr(struct i2c_client *client) 
{
	...
	APS_LOG("CM36283_REG_PS_CONF3_MS value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
#endif			
	databuf[0]= CM36283_REG_PS_CONF1_2;
	res = CM36283_i2c_master_operate(client, databuf, 0x201, I2C_FLAG_READ);
	if(res < 0)
	...
	APS_LOG("CM36283_REG_PS_CONF1_2 value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
	...
	APS_LOG("CM36283_REG_PS_THD value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
	...
	APS_LOG("CM36283_REG_INT_FLAG value_low = %x, value_high = %x\n",databuf[0],databuf[1]);
...
static void cm36283_eint_work(struct work_struct *work)
{
	...
	APS_ERR(" cm36283_eint_func time = %llu\n", eint_time);
#if 1
	res = cm36283_check_intr(obj->client);
	if(res != 0){
		goto EXIT_INTR_ERR;
...
static void cm36283_early_suspend(struct early_suspend *h)
{
		...
		if(test_bit(CMC_BIT_PS, &obj->enable)){
			...
			int res = 0;
			databuf[0]= CM36283_REG_PS_CONF3_MS;
			res = CM36283_i2c_master_operate(obj->client, databuf, 0x201, I2C_FLAG_READ);
			if(res < 0)
			{
				APS_ERR("i2c_master_send function err\n");
							}
...
static int cm36283_init_client(struct i2c_client *client)
{
	...
	databuf[1] = 0x8d;   //added by zhengdao.li for stabing L sensor
	else
	databuf[1] = 0x83;	
	databuf[2] = 0x00;
	res = CM36283_i2c_master_operate(client, databuf, 0x3, I2C_FLAG_WRITE);
	...
	databuf[0] = CM36283_REG_PS_CONF1_2;
	//databuf[1] = 0x1B;
	databuf[1] = 0xdB;  //added by zhengdao.li for elminating p sensor led flashing
...
{
	APS_FUN();
	struct cm36283_priv *obj = i2c_get_clientdata(client);//for debug operation code check in to read P-sensor register data
	int res = 0;
	if(test_bit(CMC_BIT_PS, &obj->enable)){
			u8 databuf[3];
