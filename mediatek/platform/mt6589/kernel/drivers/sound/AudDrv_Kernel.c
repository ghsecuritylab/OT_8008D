...
#ifdef JRD_IPHONE_HDST_COMPATIBILITY//Added by jrd.lipeng for iphone headset compatibility.
static int jrd_memif_count = 0;
int jrd_get_record_stat(void)
{
    PRINTK_AUDDRV("jrd.lipeng|[%s] record flag: %d", __func__, SPH_Ctrl_State.bRecordFlag);//DEBUG
    return SPH_Ctrl_State.bRecordFlag || jrd_memif_count;
}
EXPORT_SYMBOL(jrd_get_record_stat);
#endif
...
void Auddrv_Release_MemIF_Fp(struct file *fp,unsigned long arg)
{
    ...
    if(2 == arg) {
        PRINTK_AUDDRV("jrd.lipeng|[%s] memif arg: %d, jrd_memif_count--: %d", __func__, arg, jrd_memif_count);//DEBUG
        jrd_memif_count--;
    }
#endif
    PRINTK_AUDDRV("-Auddrv_Release_MemIF_Fp  = %p arg = %lu\n",pAfe_MEM_ConTrol->flip ,arg);
...
static long AudDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case START_MEMIF_TYPE:
        {
            ...
            if(arg == 2) {
                PRINTK_AUDDRV("jrd.lipeng|[%s] memif arg: %d, jrd_memif_count++: %d", __func__, arg, jrd_memif_count);//DEBUG
                //When start recording, trigger iphone headset to enable the mic of iphone headset.
                jrd_trigger_iphone_hdst();
                jrd_memif_count++;//true
            }
#endif
            break;
        }
        ...
        case SET_SPEAKER_OFF:
            ...
            mutex_unlock(&gamp_mutex);
            break;
	//ting.kang add for HAC function start 
        case SET_HAC_ON:
            PRINTK_AUDDRV("AudDrv SET_HAC_ON arg:%lu \n",arg);
            mutex_lock(&gamp_mutex);
            Sound_HAC_Turnon();
            mutex_unlock(&gamp_mutex);
            break;
        case SET_HAC_OFF:
            PRINTK_AUDDRV("AudDrv SET_HAC_OFF arg:%lu \n",arg);
            mutex_lock(&gamp_mutex);
            Sound_HAC_Turnoff();
            mutex_unlock(&gamp_mutex);
            break;
	//ting.kang add for HAC function end 
        ...
        case SET_AUDIO_STATE:
        {
            ...
            PRINTK_AUDDRV("jrd.lipeng|[%s] record flag: %d", __func__, SPH_Ctrl_State.bRecordFlag);//DEBUG
        //When start recording, trigger iphone headset to enable the mic of iphone headset.
        if(SPH_Ctrl_State.bRecordFlag == 1) {
            jrd_trigger_iphone_hdst();
        }
#endif
