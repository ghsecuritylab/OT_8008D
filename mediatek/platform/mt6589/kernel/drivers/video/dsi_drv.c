...
static bool dsi_glitch_enable = false; //baoqiang add
...
{   
    ...
    if (status.CMD_DONE)
    {
    	if (lcdStartTransfer)
        {
			...
			DBG_OnLcdDone();
			if(dsi_glitch_enable){ //legen change
				DSI_clk_HS_mode(0);
			}
        }
...
static void DSI_WaitBtaTE(void)
{
	...
	DSI_clk_HS_mode(0); //legen change
...
//legen add for compability nvk driver ic
#if 1
...
//       DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI", " DSI_RX_STA : 0x%x \n", DSI_REG->DSI_RX_STA);
	...
	if(dsi_log_on)
       ...
       else
       {
           ...
           memcpy((void*)buffer,(void*)&read_data0.byte1, buffer_size); // 2 baoqiang modify 0411
       }
#endif
