...
#ifdef JRD_IPHONE_HDST_COMPATIBILITY
static u32 GPIO_JRD_IPHONE_HDST_TRIGGER = GPIO175;//Lot0: GPIO165; Lot1: GPIO175
...
#define JRD_HDST_INSERT() (mt_get_gpio_in(GPIO_ACCDET_EINT_PIN) == CUST_EINT_ACCDET_POLARITY)
extern int jrd_get_record_stat(void);//defined in auddrv_kernel_driver.c for getting recorder state.
static int jrd_iphone_hdst_debug_on = 0;
static int jrd_iphone_hdst_delay[4] = {0, 5, 80, 5};//unit: ms. Used in jrd_trigger_iphone_hdst().
#endif

#define FIX_PR378777 //Added by jrd.lipeng for PR378777

#define JRD_FIX_CONT_ACCDET_INT 

//#define JRD_FIX_MKEY_LONG_PRESS 

//#ifdef ACCDET_MULTI_KEY_FEATURE
//#define JRD_LONG_PRESS_NEXT_SONG //Added by jrd.lipeng for one-key headset, long press is for next song.
//#endif

#define JRD_FIX_BOOTUP_WITH_HEADSET_INSERTED //Issue: earbias may be always high when boot mobile with headset inserted.
...
void accdet_eint_work_callback(struct work_struct *work)
{
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
        ...
        /* When insert a iphone headset, 
          * on martell, the current_status read here will always be 3. And 3 is the expected value. However,
          * on telsa & rav4, 
          *     when in idle mode, the current_status read here will  be 1.
          *     when in call mode, the current_status read here will  be 1 or 3, mostly 3. */
        int current_status = 0;
        int ret = -1;
        msleep(400);
        //lipeng  improve, maybe using ADC is better.
        current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); 
        ACCDET_DEBUG("[Accdet] current_status: AB=%d", current_status);//lipeng debug
        if(current_status == 3) {//When the headset is not a iphone headset, current_status should be 0 or 1.
            //if current_status is 3, it's iphone headset, then queue accdet_work explicitly.
            ret = queue_work(accdet_workqueue, &accdet_work);
             if(!ret) {
...
//EINT_PIN_PLUG_OUT
		...
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		  ...
		  cable_pin_recognition = 0;
		    ...
		    ACCDET_DEBUG("[Accdet] FSA8049 disable!\n");
		#endif
#ifdef JRD_FIX_CONT_ACCDET_INT
        /* FIX ME: if this is needed. */
        if(pmic_pwrap_read(ACCDET_PWM_WIDTH) == pmic_pwrap_read(ACCDET_PWM_THRESH)) {
            ACCDET_DEBUG("[Accdet][%s][jrd] ACCDET_PWM_WIDTH == ACCDET_PWM_THRESH\n", __func__);
...
#ifdef JRD_IPHONE_HDST_COMPATIBILITY
        //A switch to output a 2.8v voltage which is used  as a trigger for the IC of iphone headset.
        if(mt_get_gpio_in(GPIO35) == 1) {//Lot0, else lot1
            GPIO_JRD_IPHONE_HDST_TRIGGER = GPIO165;
        }
	mt_set_gpio_mode(GPIO_JRD_IPHONE_HDST_TRIGGER, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_JRD_IPHONE_HDST_TRIGGER, GPIO_DIR_OUT);
    mt_set_gpio_pull_enable(GPIO_JRD_IPHONE_HDST_TRIGGER, GPIO_PULL_DISABLE);
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_TRIGGER, JRD_IPHONE_TRIGGER_OFF);
    //GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH: switch earbias through a mos
	mt_set_gpio_mode(GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH, GPIO_DIR_OUT);
    mt_set_gpio_pull_enable(GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH, GPIO_PULL_DISABLE);
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH, JRD_EARBIAS_MOS_ON);
#endif
...
static int multi_key_detection(void)
{
	...
	while(index++ < count)
	{
                ...
                if(cur_eint_state == EINT_PIN_PLUG_OUT) {
                    /* When cur_eint_state is EINT_PIN_PLUG_OUT, disable_accdet() will be involved in 
                     * accdet_eint_work_callback() soon. So accdet registers reading should be forbidden. 
                     */
                    ACCDET_DEBUG("[Accdet][%s][jrd] cur_eint_state: %d\n", __func__, cur_eint_state);
                    send_key_event(m_key, KEYUP_FLAG);//middle key won't be processd here.
                    return (NO_KEY);
                }
#endif
...
{
    ACCDET_DEBUG("[Accdet][%s] Enter, cable_type: %d", __func__, cable_type);

    if(cable_type != HEADSET_MIC || jrd_iphone_hdst_debug_on) {
        ACCDET_DEBUG("[Accdet][%s] No headset with mic[%d] or debug on: %d. Return.", 
            __func__, cable_type, jrd_iphone_hdst_debug_on);
        return;
    }

    msleep(jrd_iphone_hdst_delay[0]);
    // Trigger gpio outputs high to trigger iphone headset IC.
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_TRIGGER, JRD_IPHONE_TRIGGER_ON);
    msleep(jrd_iphone_hdst_delay[1]);
    //disconnect mic bias
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH, JRD_EARBIAS_MOS_OFF);
    msleep(jrd_iphone_hdst_delay[2]);
    //connect mic bias
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH, JRD_EARBIAS_MOS_ON);
    msleep(jrd_iphone_hdst_delay[3]);
    //trigger off
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_TRIGGER, JRD_IPHONE_TRIGGER_OFF);
}
EXPORT_SYMBOL(jrd_trigger_iphone_hdst);
#endif
...
#define PIN_ADC_CHANNEL 5
...
#ifdef FIX_PR378777
    if(cur_eint_state == EINT_PIN_PLUG_OUT) {
        ACCDET_DEBUG("[Accdet][%s][jrd] Ignore all accdet interrupt when headset's been pulled out.", __func__);
        while(pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT)
        {
	...
	switch(accdet_status)
    {
        case PLUG_OUT:
			...
			else if(current_status == 1)
            {
			   ...
			   //recover polling set AB 00-01
				...
				//#endif
#ifdef JRD_IPHONE_HDST_COMPATIBILITY //Added by jrd.lipeng for iphone headset compatibility.
                    /* Sometimes AB=1 when insert a iphone headset in call mode though mostly AB will be 3. */
                    if(call_status == 2 || jrd_get_record_stat()) {
                        //msleep(150);
                        jrd_trigger_iphone_hdst();
                    }
#endif
            }
            ...
            {
#ifdef JRD_IPHONE_HDST_COMPATIBILITY //Added by jrd.lipeng for iphone headset compatibility.
                //Mostly when insert a iphone headset in call mode current_status will be 3.
                jrd_hdst_insert = JRD_HDST_INSERT();
                ACCDET_DEBUG("[Accdet][%s] jrd_hdst_insert: %d", __func__, jrd_hdst_insert);
                if(jrd_hdst_insert) {
                    ACCDET_DEBUG("[Accdet][%s] jrd_hdst_insert is true & current_status is 3, "
                        "so I think it's a 4-nodes iphone headset", __func__);
                    accdet_status = MIC_BIAS;
                    cable_type = HEADSET_MIC;
                    ...
                    /* When need use headset mic,  trigger the IC of iphone headset to let its mic work. */
                    if(call_status == 2 || jrd_get_record_stat()) {
                        //msleep(150);
                        jrd_trigger_iphone_hdst();
                    }
	    ...
	    //ALPS00038030:reduce the time of remote button pressed during incoming call
            ...
            if(current_status == 0)
            {
#ifdef JRD_FIX_BOOTUP_WITH_HEADSET_INSERTED
            #ifdef ACCDET_MULTI_KEY_FEATURE
		    ...
		    {	
                ...
                if(cur_eint_state == EINT_PIN_PLUG_OUT) {
                    /* When cur_eint_state is EINT_PIN_PLUG_OUT, disable_accdet() will be involved in 
                     * accdet_eint_work_callback() soon. So accdet registers writing should be forbidden. 
                     * These writing are removed to accdet_eint_work_callback().
                     */
                    ACCDET_DEBUG("[Accdet][%s][jrd] cur_eint_state: %d\n", __func__, cur_eint_state);
                    return;
                }
#endif
#ifdef JRD_FIX_MKEY_LONG_PRESS
                /* When processing a long middle key press, 
                  * 1. report the key;
                  * 2. wait until the current_status becomes non-0, then do the registers writing.*/
                if(LONG_MD == multi_key) {
                    ACCDET_DEBUG("[Accdet][%s][jrd] Long press middle (0x%x)\n", __func__, multi_key);
                    if(call_status == 0)
#ifdef JRD_LONG_PRESS_NEXT_SONG
                    {
                    ...
                    while(current_status == 0) {
                        ACCDET_DEBUG("[Accdet][%s][jrd] wait until current_status(%d) is non-zero.\n", 
...
//#endif
          ...
          {
#ifdef JRD_IPHONE_HDST_COMPATIBILITY //Added by jrd.lipeng for iphone headset compatibility.
                /*When a hook key of iphone headset is pressed, the program will run here.*/
                jrd_hdst_insert = JRD_HDST_INSERT();
                ACCDET_DEBUG("[Accdet][%s] jrd_hdst_insert???: %d", __func__, jrd_hdst_insert);
                if(jrd_hdst_insert) {
                    accdet_status = MIC_BIAS;		
		...
		//ALPS00038030:reduce the time of remote button pressed during incoming call
         //solution: reduce hook switch debounce time to 0x400
            ...
            else if(current_status == 3)
            {
                ...
                /*When a hook key of iphone headset is pressed, the program will run here.*/
                jrd_hdst_insert = JRD_HDST_INSERT();
                ACCDET_DEBUG("[Accdet][%s] jrd_hdst_insert2: %d", __func__, jrd_hdst_insert);
                if(jrd_hdst_insert) {
                    accdet_status = MIC_BIAS;		
                    cable_type = HEADSET_MIC;
...
int accdet_irq_handler(void)
{
	...
	//}
	//mutex_unlock(&accdet_eint_irq_sync_mutex);
#ifndef JRD_FIX_BOOTUP_WITH_HEADSET_INSERTED//Removed by jrd.lipeng: Earbias may be always high when boot mobile with headset inserted.
	#ifdef ACCDET_MULTI_KEY_FEATURE
    if (accdet_status == MIC_BIAS){
    pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings.pwm_width);
    pmic_pwrap_write(ACCDET_PWM_THRESH, cust_headset_settings.pwm_width);
...
#ifdef JRD_IPHONE_HDST_COMPATIBILITY
                        /* The earbias outputs stably after call begins active, then trigger the iphone headset IC 
                          * with a 2.8v pulse to let iphone headset work . */
                        if(call_status == 2) {// 2: active
                            jrd_trigger_iphone_hdst();
                        }
#endif
			break;
...
#ifdef JRD_IPHONE_HDST_COMPATIBILITY//Added by jrd.lipeng for iphone headset compatibility debug
static void jrd_iphone_hdst_trigger_mos_on(int trig_on, int mos_on)
{
    int trig_gpio_output = 0;
    int mos_gpio_output = 0;
    ...
    trig_gpio_output = trig_on ? JRD_IPHONE_TRIGGER_ON : JRD_IPHONE_TRIGGER_OFF;
    mos_gpio_output = mos_on ? JRD_EARBIAS_MOS_ON : JRD_EARBIAS_MOS_OFF;
    
    ACCDET_DEBUG("[Accdet][%s] trigger_on:mos_on(gpio output)=%d:%d(%d:%d)", 
                                                  __func__, trig_on, mos_on, trig_gpio_output, mos_gpio_output);
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_TRIGGER, trig_gpio_output);
    mt_set_gpio_out(GPIO_JRD_IPHONE_HDST_EARBIAS_SWITCH, mos_gpio_output);
}

static int jrd_iphone_hdst_debug_parse(const char *buf)
{
    char *str = buf;
    char *item = NULL;
    int trig = 0, mos = 0;
    ...
    ACCDET_DEBUG("[Accdet][%s] jrd.lipeng|input: %s", __func__, str);//debug

    //eg: 00:d5:10:d100:01
    if(str[0] == 'D') {//debug on: ignore jrd_trigger_iphone_hdst()
        str++;
        jrd_iphone_hdst_debug_on = 1;
        ACCDET_DEBUG("[Accdet][%s] jrd_iphone_hdst_debug_on: %d", jrd_iphone_hdst_debug_on);
    } else if(str[0] == 'S') {//setting delay times in jrd_trigger_iphone_hdst()
        jrd_iphone_hdst_debug_on = 0;
        ACCDET_DEBUG("[Accdet][%s] jrd_iphone_hdst_debug_on: %d", jrd_iphone_hdst_debug_on);
        if(strlen(str) == 1) return 0;//Just set jrd_iphone_hdst_debug_on to 0
        str++;
        
        i = 0;
        while(item = strsep(&str, ":")) {
            sscanf(item, "%u", &delay); 
            jrd_iphone_hdst_delay[i++] = delay;
            if(i > 4) {
                ACCDET_DEBUG("[Accdet][%s] Invalid settings!", __func__);
                break;
            }
            ACCDET_DEBUG("[Accdet][%s] jrd_iphone_hdst_delay[%d]: %d", __func__, i-1 , jrd_iphone_hdst_delay[i-1]);
        }
        return 0;
    }
    
    while(item = strsep(&str, ":")){
        if(item[0] == 'd') {//delay time(unit ms)
            sscanf(item + 1, "%u", &delay); 
            ACCDET_DEBUG("[Accdet][%s] jrd.lipeng|msleep: %dms", __func__, delay);//debug
            msleep(delay);
        } else {//gpio output setting
            if(item[0] == '0') {
                trig = 0;
            } else {
                trig = 1;
            ...
            jrd_iphone_hdst_trigger_mos_on(trig, mos);
        }
    }
...
static ssize_t jrd_store_accdet_iphone_hdst_debug(struct device_driver *ddri, const char *buf, size_t count)
{
    jrd_iphone_hdst_debug_parse(buf);
...
#ifdef JRD_IPHONE_HDST_COMPATIBILITY//Added by jrd.lipeng for iphone headset compatibility debug
static DRIVER_ATTR(iphone_hdst_debug,      S_IWUSR | S_IRUGO, NULL,         jrd_store_accdet_iphone_hdst_debug);
#endif

/*----------------------------------------------------------------------------*/
static struct driver_attribute *accdet_attr_list[] = {
#ifdef JRD_IPHONE_HDST_COMPATIBILITY//Added by jrd.lipeng for iphone headset compatibility debug
        &driver_attr_iphone_hdst_debug,
#endif
