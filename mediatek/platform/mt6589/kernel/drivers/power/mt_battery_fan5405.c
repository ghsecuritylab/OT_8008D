...
static int max_charge_temperature = MAX_CHARGE_TEMPERATURE;
...
#endif
#define FAN5405_AC_CHARGING_CURRENT_950
...
#define CHG_MIN_VALID_BATT_ID  1200
#define CHG_MAX_VALID_BATT_ID  2500
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);
void check_battery_id(void);
#endif

unsigned long turn_on_off_charging = 1;		//mingquan.lai add interface for charging test(60%) in factory mode
...
static int  batteryTempDiscardCnt = 0;		// discard the first value.


static int batteryTempSum = 0;
static int batteryTempBuffer[BATTERY_TEMP_AVERAGE_SIZE];	// only use BATTERY_TEMP_AVERAGE_SIZE . do not use sizeof
//static int batteryTempBuffer[BATTERY_AVERAGE_SIZE];
static int batteryTempIndex = 0;
...
static void mt6320_battery_update(struct mt6320_battery_data *bat_data)
{
    ...
    if( (upmu_is_chr_det()==KAL_TRUE) && (!g_Battery_Fail) && (g_Charging_Over_Time==0) && turn_on_off_charging)  //modify by tao.zhang. for factory charging test.2012-12-26
    {     
        if ( BMT_status.bat_exist )                
        {
            /* Battery Full */
...
#define TEMP_OFFSET_GT_ZERO 30		// 30 + (-20)
...
BATT_TEMPERATURE Batt_Temperature_Table[] = {
 ...
 { 60,2970}
    ...
    else
    {
		...
		tbatt_offset = (((Res-RES2)*(TMP1+TEMP_OFFSET_GT_ZERO))+((RES1-Res)*(TMP2+TEMP_OFFSET_GT_ZERO)))/(RES1-RES2);
		//printk("TBatt_Value %d tbatt_offset %d\n", TBatt_Value, tbatt_offset-TEMP_OFFSET_GT_ZERO);
		TBatt_Value = tbatt_offset-TEMP_OFFSET_GT_ZERO;
    }
    ...
    if(temp_init_flag==0)
    {
        if(g_enable_high_vbat_spec == 1)
        {
            if(g_pmic_cid == 0x1020)
                fan5405_config_interface_liao(0x06,0x78);    //mingquan.lai set ISAFE 4.36V 20130101
            else
		...
		#ifdef SWITCH_CHARGER_UNSTANDARD_1A
		    fan5405_config_interface_liao(0x01,0xf9); //set Iinlim to nolimit
		#else
        ...
        {
            if(g_pmic_cid == 0x1020)
                fan5405_config_interface_liao(0x02,0xae);    //mingquan.lai set charger Vout Float Voltage to 4.36V 20130101
            else    
...
#ifdef SWITCH_CHARGER_UNSTANDARD_1A
            fan5405_config_interface_liao(0x04,0x4b);  //set Iterm to 194ma,set Iochr to 950ma
#else
...
void fan5405_set_ac_current(void)
{
    ...
    if(g_low_power_ready == 1)
        ...
        reg_set_value += 0x09;  //mingquan.lai set the current used for charging termination to 97mA 20130101
            ...
            {    
                ...
                else 
                {
                    ...
                    if(g_temp_status == TEMP_POS_10_TO_POS_45)
                    {
                        ...
                        {
                            if(g_pmic_cid == 0x1020)
                                fan5405_config_interface_liao(0x02,0xaa);	//mingquan.lai set charger Vout Float Voltage to 4.34V
                            else
                                fan5405_config_interface_liao(0x02,0xaa);
                        }
                        else
...
#else            
                fan5405_config_interface_liao(0x01,0x78);
                fan5405_config_interface_liao(0x04,0x09); //mingquan.lai set USB charging current 550mA,charging termination to 97mA 20130101
#endif                
                if (Enable_BATDRV_LOG == 1) {
...
#else        
#ifdef SWITCH_CHARGER_UNSTANDARD_1A
            fan5405_config_interface_liao(0x01,0xf8);  //set Iinlim to nolimit
#else
            fan5405_config_interface_liao(0x01,0x78);
#endif
#endif            
...
#ifdef SWITCH_CHARGER_UNSTANDARD_1A
    fan5405_config_interface_liao(0x01,0xfc);      
#else
    fan5405_config_interface_liao(0x01,0xbc); 
#endif
...
int BAT_CheckBatteryStatus_fan5405(void)
{
    ...
    //-----------------------------------------------------------------------------
        ...
        g_bat_temperature_pre = BattVoltToTemp(bat_temperature_volt);
    }
    ...
    else
    {
        ...
        if (!batteryBufferFirst)
        {
			...
			for (i=0; i<BATTERY_TEMP_AVERAGE_SIZE; i++) {
				batteryTempBuffer[i] = g_bat_temperature_pre;
			}
			batteryTempSum = g_bat_temperature_pre * BATTERY_TEMP_AVERAGE_SIZE;
			batteryTempDiscardCnt = 0;
			batteryTempIndex = 0;
        }

		// discard the first temp value. start
		batteryTempDiscardCnt++;
		if (batteryTempDiscardCnt == 2){	
			for (i=0; i<BATTERY_TEMP_AVERAGE_SIZE; i++) {
				batteryTempBuffer[i] = g_bat_temperature_pre;
			}
		...
		if (batteryTempDiscardCnt >= BATTERY_AVERAGE_SIZE){
			batteryTempDiscardCnt = BATTERY_AVERAGE_SIZE;
		}
		// discard the first temp value. end
		...
		batteryTempBuffer[batteryTempIndex] = g_bat_temperature_pre;
		batteryTempSum = 0;
		for(i=0; i<BATTERY_TEMP_AVERAGE_SIZE; i++){
			batteryTempSum += batteryTempBuffer[i];
		}
        // 0.5 then = 1; 0.4 then = 0;
        batteryTempSum += TEMP_OFFSET_GT_ZERO * BATTERY_TEMP_AVERAGE_SIZE;
		batteryTempSum = (batteryTempSum * 10 + 5 * BATTERY_TEMP_AVERAGE_SIZE)/BATTERY_TEMP_AVERAGE_SIZE;
		BMT_status.temperature = batteryTempSum/10 - TEMP_OFFSET_GT_ZERO;
		#if 0
		printk("BMT_status.temperature 0.5then add 1    %d\n", BMT_status.temperature);
		printk("BATTERY_TEMP_AVERAGE_SIZE %d <MIN_CHARGE_TEMPERATURE %d >=MAX_CHARGE_TEMPERATURE %d\n",BATTERY_TEMP_AVERAGE_SIZE, MIN_CHARGE_TEMPERATURE, MAX_CHARGE_TEMPERATURE);
		printk("batteryTempIndex %d \n", batteryTempIndex);
		for(i=0; i<BATTERY_TEMP_AVERAGE_SIZE; i++){
			printk("%d %d\n",i, batteryTempBuffer[i]);
		}
		#endif
		...
		batteryTempIndex++;
        if (batteryTempIndex >= BATTERY_TEMP_AVERAGE_SIZE)
            batteryTempIndex = 0;
	...
	if ((BMT_status.temperature > (MIN_CHARGE_TEMPERATURE+2)) && (BMT_status.temperature < (max_charge_temperature-3)))
	{
		 if ((BATTERY_LOW_TEMP == BMT_status.charger_protect_status)||(BATTERY_OVER_TEMP == BMT_status.charger_protect_status)	){
		 	BMT_status.charger_protect_status = 0;
		 }
	}
#endif
...
{    
    ...
    {   
	   if((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == NONSTANDARD_CHARGER)){
               printk(KERN_ALERT "jrd %s: ac disconnect detected!\n",__func__);
               mt6320_ac_update(&mt6320_ac_main);
               msleep(500);
           	}
          else if((BMT_status.charger_type == STANDARD_HOST) || (BMT_status.charger_type == CHARGING_HOST)){
               printk(KERN_ALERT "jrd %s: usb disconnect detected!\n",__func__);
               mt6320_usb_update(&mt6320_usb_main);
               msleep(500);
          	} 
    ...
    else
    {
		...
		if(turn_on_off_charging)		//mingquan.lai add interface for charging test(60%) in factory mode
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case Set_Charger_Current: /* For Factory Mode*/
            ...
            break;
        case TURN_OFF_CHARGING_FOR_FACTORY:  //mingquan.lai merge from Rav4 for factory charging function
		pchr_turn_off_charging_fan5405();
...
static ssize_t show_Charger_Off_Value(struct device *dev,struct device_attribute *attr, char *buf)
{
    xlog_printk(ANDROID_LOG_DEBUG,"Power/Battery", "max_charge_temperature : %d\n", max_charge_temperature);
    return sprintf(buf, "%u\n", max_charge_temperature);
}
static ssize_t store_Charger_Off_Value(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    sscanf(buf, "%u", &max_charge_temperature);
    xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "call state = %d\n",max_charge_temperature);
    return size;
}
static DEVICE_ATTR(Charger_Off_Value, 0664, show_Charger_Off_Value, store_Charger_Off_Value);
...
///////////////////////////////////////////////////////////////////////////////////////////
//// Create File For EM : turn_on_off_charging     mingquan.lai add interface for charging test(60%) in factory mode
///////////////////////////////////////////////////////////////////////////////////////////
static ssize_t show_turn_on_off_charging(struct device *dev,struct device_attribute *attr, char *buf)
{
    xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] turn_on_off_charging : %d\n", turn_on_off_charging);
    return sprintf(buf, "%u\n", turn_on_off_charging);
}
static ssize_t store_turn_on_off_charging(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned long val = simple_strtoul(buf,NULL,10);
	if(val == 1){
		turn_on_off_charging = 1;
		pchr_turn_on_charging_fan5405();
	}
	else if(val == 0){
		turn_on_off_charging = 0;
		pchr_turn_off_charging_fan5405();
	}
	else
    	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] Invalid Write value\n");    
    return size;
}
static DEVICE_ATTR(turn_on_off_charging, 0664, show_turn_on_off_charging, store_turn_on_off_charging);
...
static int mt6320_battery_probe(struct platform_device *dev)    
{
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_turn_on_off_charging);    //mingquan.lai add interface for charging test(60%) in factory mode
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charger_Off_Value);
