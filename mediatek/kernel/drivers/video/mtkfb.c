...
extern const LCM_DRIVER  *lcm_drv;//legen 20130405
...
/*
 * ---------------------------------------------------------------------------
 * Sysfs interface: /sys/devices/platform/mtkfb.0
 * ---------------------------------------------------------------------------
 */

//baoqiang add for adb shell to get display mode
//extern bool LCM_MODE;
//extern int *lcm_read_registers(void);
/*
static ssize_t show_display_mode(struct device *dev,struct device_attribute *attr, char *buf)
{
	printk("[qbq]get display mode\n");
...
static ssize_t show_lcd_registers(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("[qbq] snatch the registers of lcd!\n");
	if(lcm_drv->read_registers == NULL)
	{
		printk("[qbq]--not defined read registers interface\n");
		return sprintf(buf,"%s\n","not define this interface!");
	}
		
	p=lcm_drv->read_registers();
	
	for(i=0;i<16;i++)
		buffer[i]=*(p++);
	
	temp += sprintf(temp,"%s","0x0a: ");
	for(i=0;i<7;i++)
	{
		temp += sprintf(temp,"%x  ",buffer[i]);
	}
...
int log_start_fb = 0;
static ssize_t store_open_lcd_log_en(struct device *dev, struct device_attribute *attr, char *buf, size_t count)
{
	printk("[qbq] start open the lcd read register log!\n");
	char *pvalue=NULL;
	unsigned int log;
	if(buf!=NULL && count!=0)
	{
		log=simple_strtoul(buf,&pvalue,10);
		if(log)
			log_start_fb=1;
		else
			log_start_fb=0;
	}
	printk("[qbq] open log of reading lcd registers:1-open,0-close value is:%d\n",log_start_fb);

	return count;
}
//static DEVICE_ATTR(mode, S_IRUGO, show_display_mode, NULL);
static DEVICE_ATTR(regs, S_IRUGO, show_lcd_registers, NULL);
static DEVICE_ATTR(log_en, S_IWUSR | S_IRUGO, NULL, store_open_lcd_log_en);

static struct device_attribute *mtk_attr_list[] = {
	//&dev_attr_mode,
	&dev_attr_regs,
	&dev_attr_log_en,
};
...
static int mtkfb_register_sysfs(struct mtkfb_device *fbdev)
{
    ...
    int ret,id;
	int num = (int)(sizeof(mtk_attr_list)/sizeof(mtk_attr_list[0]));

	for(id=0; id<num; id++)
	{	
    	ret = device_create_file(fbdev->dev, mtk_attr_list[id]);
		if(ret)
		{
			printk("[LCD]device_create_file (%s) fail!\n",mtk_attr_list[id]->attr.name);
		}
	}
