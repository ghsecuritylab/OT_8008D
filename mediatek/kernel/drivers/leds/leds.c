...
static int led_set_pwm(int pwm_num, struct nled_setting* led)
{
	...
	switch (led->nled_mode)
	{
		case NLED_OFF :
			if(pwm_num == PWM1)
			pwm_setting.PWM_MODE_OLD_REGS.THRESH = 100;
			else
		...
		case NLED_BLINK :
			...
			pwm_setting.PWM_MODE_OLD_REGS.THRESH = led->blink_on_time*MIN_FRE_OLD_PWM / div_array[time_index];
	}
...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	else if (pmic_type == MT65XX_LED_PMIC_BUTTON) 
	{
		if (level) 
		{
			hwBacklightKPLEDTuning(0x19, 0x0);
	...
	if(buf != NULL && size != 0)
		{
			...
			if(notify_led == 1)
				{
					mt_set_gpio_out(GPIO28,GPIO_OUT_ONE);
					notify_led_enable = 1;
			...
			if(notify_led == 0)
				{
					mt_set_gpio_out(GPIO28,GPIO_OUT_ZERO);
					notify_led_enable = 0;
