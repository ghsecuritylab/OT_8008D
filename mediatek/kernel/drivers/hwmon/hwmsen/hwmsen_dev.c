...
#define MAX_CHOOSE_ALSPS_NUM 2  //added by zhengdao
...
static struct sensor_init_info* alsps_init_list[MAX_CHOOSE_ALSPS_NUM]= {0}; 
#endif
...
static int hwmsen_enable(struct hwmdev_object *obj, int sensor, int enable)
{
	...
	if(enable == 1)
	{
		...
		if(sensor == ID_PROXIMITY)				//set the default value of ps as 1 which means far away ,do not modify
		{
			obj_data.sensors_data[ID_PROXIMITY].values[0] = 1;
			obj_data.sensors_data[ID_PROXIMITY].value_divide = 1;
		}
...
int hwmsen_alsps_remove(struct platform_device *pdev)
{
	...
	printk("liang_hwmsen_alsps_remove");
    return 0;
}
...
static int __init hwmsen_init(void) 
{
    ...
    if(platform_driver_register(&gsensor_driver))
	{
		...
		{
			HWM_ERR("failed to register plsensor driver");
			return -ENODEV;
		}
#endif
